...
MenuScreen::MenuScreen() :selectedItemIndex(0)
{
	...
	menu[0].setPosition(sf::Vector2f(1000 / 2, 480 / (MAX_NUMBER_OF_ITEMS + 1) * 1));

	menu[1].setFont(this->font);
	menu[1].setFillColor(sf::Color::White);
	menu[1].setString("Creator");
	menu[1].setPosition(sf::Vector2f(1000 / 2, 480 / (MAX_NUMBER_OF_ITEMS + 1) * 2));

	menu[2].setFont(this->font);
	menu[2].setFillColor(sf::Color::White);
	menu[2].setString("Exit");
	menu[2].setPosition(sf::Vector2f(1000 / 2, 480 / (MAX_NUMBER_OF_ITEMS + 1) * 3));
}

void MenuScreen::draw(sf::RenderWindow* window)
{
	sf::RectangleShape rect;
	rect.setSize({ 1040.0f, 480.0f });
